...
class OCLgenerator():
    ...
    def gen_score(self, predictions, references, metric):
        ...
        for i in range(len(predictions)):
            if (metric=='pft'):
                if(predictions[i].replace('\n','').replace('\r','').replace(' ','')==
                   references['OCL'][i].replace('\n','').replace('\r','').replace(' ','')):
                    score=100
                else:
                    score=0              
            elif (metric == self.rouge):
                score = self.rouge.get_scores([predictions[i]],[references['OCL'][i]])
                score = score[0]['rouge-l']['f']
                score = score*100
            elif (metric.name == 'sacrebleu'):
                score = metric.compute(predictions=[predictions[i]], references=[[references['OCL'][i]]])
                score = score['score']
            elif(metric.name=='key_match_metric'):
                score = metric.compute(predictions=predictions[i], references=references['OCL'][i])
                score = score['score']                
            score=round(score,3)
            scores.append(score)
    ...
    def gen_evaluate(self, with_predict=False, eva_predict=False):
        ...
        custom_scores = {
            ...
            'name': 'key_match'}
        perfect_scores = {
            'score' : self.gen_score(predictions=generated, references=dataset, metric='pft'),
            'name': 'Acc'}
        rouge_scores = {
            'score': self.gen_score(predictions=generated, references=dataset, metric=self.rouge),
            'name':'Rouge-l'}
        ...
        if eva_predict:
            ...
            result = pd.DataFrame(
                {'NL': dataset['NL'], 'OCL': dataset['OCL'], 'Candidate': generated,
                 ...
                 'Predict': pred_scores, 'Acc':perfect_scores['score'], 'Rouge-l':rouge_scores['score']})

        else:
            result = pd.DataFrame(
                {'NL': dataset['NL'], 'OCL': dataset['OCL'], 'Candidate': generated,
                 'BLEU': bleu_scores['score'], 'Key_metric': custom_scores['score'],
                 'Acc':perfect_scores['score'], 'Rouge-l':rouge_scores['score']})
